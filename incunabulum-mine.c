...
A ga(t,r,d)I *d;{A z=(A)ma(sizeof(struct a));z->p=ma(tr(r,d));z->t=t,z->r=r,mv(z->d,d,r); R z;}
...
/* create an array of w->t=1 (transposed?), whose pointer wraps the input array */
...
/* z->r:
 ...
 z->n = product of 1st z->r elements of a->p.  so it's a->p[0] if a is scalar,
 else product of a->d[0] elements?? WTF.  this mostly deals with the `p` stuff
 that I don't understand very well just yet. */
...
A noun(c){A z;if(c<'0'||c>'9')R 0;z=ga(0,0,0);*z->p=c-'0';R z;}
A verb(c){I i=0;for(;vt[i];)if(vt[i++]==c)R i;R 0;}
